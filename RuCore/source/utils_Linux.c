...
bool RukkyLoadWayland(uptr** function_array) {
    void *handle = dlopen(
        "/lib/x86_64-linux-gnu/libwayland-egl.so",
        RTLD_NOW
    );
...
bool RukkyLoadX11(uptr** function_array) {
    ...
    dlfuncptr(function_array,  0, "glXChooseFBConfig");             // New Window
    dlfuncptr(function_array,  1, "glXCreateNewContext");           // New Window
    dlfuncptr(function_array,  2, "glXCreateWindow");               // New Window
    dlfuncptr(function_array,  3, "glXDestroyContext");             // Loop
    dlfuncptr(function_array,  4, "glXDestroyWindow");              // Loop
    dlfuncptr(function_array,  5, "glXGetProcAddress");             // New Window
    dlfuncptr(function_array,  6, "glXGetVisualFromFBConfig");      // New Window
    dlfuncptr(function_array,  7, "glXMakeContextCurrent");         // New Window & Loop
    dlfuncptr(function_array,  8, "glXSwapBuffers");                // Loop
    dlfuncptr(function_array,  9, "XCloseDisplay");                 // Loop
    dlfuncptr(function_array, 10, "XCreateColormap");               // New Window
    dlfuncptr(function_array, 11, "XCreateWindow");                 // New Window
    dlfuncptr(function_array, 12, "XDestroyWindow");                // Loop
    dlfuncptr(function_array, 13, "XFlush");                        // New Window
    dlfuncptr(function_array, 14, "XFreeColormap");                 // Loop
    dlfuncptr(function_array, 15, "XInternAtom");                   // Loop
    dlfuncptr(function_array, 16, "XMapWindow");                    // New Window
    dlfuncptr(function_array, 17, "XNextEvent");                    // Loop
    dlfuncptr(function_array, 18, "XOpenDisplay");                  // New Window
    dlfuncptr(function_array, 19, "XPending");                      // Loop
    dlfuncptr(function_array, 20, "XSetWMProtocols");               // Loop
    dlfuncptr(function_array, 21, "XStoreName");                    // New Window
